...
class LocalizationDataset(Dataset):
    ...
    def get_pos_samples(self, anchor_idx):
        filtered_samples = [sample for idx, sample in enumerate(self.samples) if
                            idx != anchor_idx]  # all candidate exlude itself.
    ...
    for i in range(row):
        ...
        for motif in motifs:
            if not pd.isnull(motif):
                ...
                motif_left, motif_right, type_protein, targets = fix_sample(motif_left, motif_right, label, label2idx,
                                                                            type_protein, targets)
...
def prepare_dataloaders(configs, valid_batch_number, test_batch_number):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=configs.valid_settings.batch_size, shuffle=False,
                                 collate_fn=custom_collate)
